...
final class DataQualityAgent(apiKey: Option[String], model: String) {
  ...
  def analyze(rows: Vector[Row], expectedSchema: Option[Value]): Value = {
    ...
    val semanticContext = inferSemantics(profile)
    val checkPlan = buildCheckPlan(semanticContext, expectedSchema)
    val execution = runChecks(checkPlan)
  ...
  private def inferSemantics(profile: Value): Value =
    apiKey match {
  ...
  private def runChecks(plan: Value): Value = {
  ...
  private def assess(profile: Value, semantic: Value, plan: Value, issues: Vector[DataIssue], score: Int): Value =
    apiKey match {
  ...
  private def heuristicSemantics(profile: Value): Value = {
    ...
    val semantics = Obj.from(columns.map { column =>
      ...
      val allowedValues: Value =
        if (lowered.contains("market")) Arr.from(ProjectDefaults.AllowedMarkets.map(Str(_)))
        else Null
    ...
    Obj(
      ...
      "priority_checks" -> Arr(
        ...
        Obj("name" -> "allowed_values", "params" -> Obj("column" -> "market", "values" -> Arr.from(ProjectDefaults.AllowedMarkets.map(Str(_)))), "priority" -> "MEDIUM", "reason" -> "Code columns should stay within expected values.")
      )
    )
  }

  private def heuristicAssessment(profile: Value, semantic: Value, plan: Value, issues: Vector[DataIssue], score: Int): Value = {
  ...
  private def callAssessment(profile: Value, semantic: Value, plan: Value, issues: Vector[DataIssue], score: Int, key: String): Option[Value] = {
    val payload = Obj(
      "quality_score" -> score,
      "profile" -> profile,
  ...
  private def parseJsonObject(raw: String, requiredKeys: Set[String]): Option[Obj] =
    Try(ujson.read(raw)).toOption.collect { case obj: Obj if requiredKeys.subsetOf(obj.value.keySet) => obj }
  ...
  private def normalizeAssessment(obj: Obj): Unit = {
    Seq("key_issues", "root_causes", "recommendations", "next_checks", "suggested_fixes").foreach { key =>
      obj.value.get(key).foreach {
        ...
        case Str(text) => obj(key) = Arr.from(text.split("\n").toVector.map(_.trim).filter(_.nonEmpty).map(Str(_)))
        case other => obj(key) = Arr(Str(other.render()))
      }
    }
  }
  ...
  private def dedupeChecks(checks: Vector[CheckSpec]): Vector[CheckSpec] =
    checks.groupBy(spec => (spec.name, ujson.write(spec.params))).values.map(_.head).toVector.sortBy(spec => (severityRank(spec.priority.toUpperCase), spec.name))
  ...
  private def normalizeCheck(value: Value): Option[CheckSpec] = value match {
    case obj: Obj =>
      ...
      nameOpt.map { name =>
        ...
        CheckSpec(name.trim.toLowerCase.replaceAll("\\s+", "_"), params, priority, reason)
      }
  ...
  private def normalizeColumnSemantic(column: String, value: Value): Option[Obj] = value match {
    ...
    case Str(text) =>
      ...
      val allowedValues: Value =
        if (semanticType == "exchange_code") Arr.from(ProjectDefaults.AllowedMarkets.map(Str(_)))
        else Null
  ...
  private def runDuplicateRowsCheck(spec: CheckSpec): Vector[DataIssue] = {
    ...
    Option.when(duplicateCount > 0)(DataIssue("DUPLICATE_ROWS", "__row__", "HIGH", duplicateCount, s"Dataset contains $duplicateCount duplicated row(s).", Obj())).toVector
  }

  private def runInfiniteValuesCheck(spec: CheckSpec): Vector[DataIssue] =
    datasetColumns.flatMap { column =>
      val infiniteCount = currentRows.map(_.values.getOrElse(column, "")).count(isInfinity)
      Option.when(infiniteCount > 0)(DataIssue("INFINITE_VALUES", column, "HIGH", infiniteCount, s"Column '$column' contains $infiniteCount infinite value(s).", Obj()))
    }
  ...
  private def runBlankStringsCheck(spec: CheckSpec): Vector[DataIssue] =
    datasetColumns.flatMap { column =>
      ...
      Option.when(blankCount > 0)(DataIssue("EMPTY_STRINGS", column, "MEDIUM", blankCount, s"Column '$column' contains $blankCount blank string value(s).", Obj()))
    }
  ...
  private def runOutlierCheck(spec: CheckSpec): Vector[DataIssue] = {
    ...
    targetColumns.flatMap { column =>
      val outlierCount = countOutliers(numericValues(column))
      Option.when(outlierCount > 0)(DataIssue("OUTLIERS", column, "MEDIUM", outlierCount, s"Column '$column' has $outlierCount value(s) outside the IQR bounds.", Obj()))
    }
  }
  ...
  private def runNonNegativeCheck(spec: CheckSpec): Vector[DataIssue] =
    spec.params.value.get("column").toVector.flatMap { columnValue =>
      ...
      Option.when(negativeCount > 0)(DataIssue("NEGATIVE_VALUES", column, "MEDIUM", negativeCount, s"Column '$column' has $negativeCount unexpected negative value(s).", Obj("min_value" -> Num(values.min)))).toVector
    }
  ...
  private def runAllowedValuesCheck(spec: CheckSpec): Vector[DataIssue] = {
    ...
    (for {
      ...
      Option.when(invalid.nonEmpty)(DataIssue("ENUM_VIOLATION", column, "MEDIUM", invalid.size, s"Column '$column' contains values outside the allowed set.", Obj("sample_invalid_values" -> Arr.from(invalid.distinct.take(5).map(Str(_)))))).toVector
    }).flatten
  }
  ...
  private def runCompositeUniquenessCheck(spec: CheckSpec): Vector[DataIssue] = {
    ...
    if (columns.isEmpty) Vector.empty
      ...
      val duplicates = currentRows.map(row => columns.map(col => row.values.getOrElse(col, "")).mkString("||")).filter(_.nonEmpty).groupBy(identity).values.map(_.size - 1).filter(_ > 0).sum
      Option.when(duplicates > 0)(DataIssue("COMPOSITE_DUPLICATES", columns.mkString(","), "HIGH", duplicates, s"Columns ${columns.mkString("(", ", ", ")")} contain duplicate combinations.", Obj())).toVector
    }
  }
  ...
  private def runIdentifierDuplicateCheck(spec: CheckSpec): Vector[DataIssue] =
    spec.params.value.get("column").toVector.flatMap { columnValue =>
      ...
      Option.when(duplicates > 0)(DataIssue("IDENTIFIER_DUPLICATES", column, "HIGH", duplicates, s"Identifier-like column '$column' contains duplicate values.", Obj())).toVector
    }
